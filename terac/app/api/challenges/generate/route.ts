    ...
    try {
      ...
      if (uniqueContents.length >= 10) {
        ...
        if (challengeType === 'Bug') {
            prompt = `Generate a 10-line text snippet (code) that has exactly one hidden logical error. 
            Return ONLY JSON in the following format, with no markdown formatting around it:
            {
              "code": "the code text here",
              "answer": "the exact line number or explanation of the bug",
              "language": "javascript"
            }`;
        } else if (challengeType === 'Efficiency') {
